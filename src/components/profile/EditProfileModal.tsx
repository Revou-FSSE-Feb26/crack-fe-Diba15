  ...
  const content = useMemo(
    () => (
      <div className="space-y-4">
        <div>
          <label htmlFor="profile-name" className="mb-1.5 block text-sm font-semibold text-content">
            Nama
          </label>
          <Input
            id="profile-name"
            ...
            {...register("name", {
              ...
              validate: (value) => value.trim().length > 0 || "Nama wajib diisi",
            })}
          >
        ...
        <div>
          <label htmlFor="profile-bio" className="mb-1.5 block text-sm font-semibold text-content">
            Bio
          </label>
          <textarea
            id="profile-bio"
        ...
        <div>
          <label htmlFor="profile-price" className="mb-1.5 block text-sm font-semibold text-content">
            Harga Mulai Dari
          </label>
          <Input
            id="profile-price"
            ...
            {...register("base_price_idr", {
              ...
              validate: (value) => Number.isFinite(value) || "Harga harus berupa angka",
            })}
          >
            <Wallet className="h-5 w-5 text-gray-400" />
          </Input>
          {errors.base_price_idr && (
            <p className="mt-1 text-xs text-danger">{errors.base_price_idr.message}</p>
          )}
        </div>
        ...
        <label className="flex items-start gap-3 rounded-xl border border-content/10 bg-content/5 px-4 py-3 cursor-pointer">
          ...
          <span>
            ...
            <span className="mt-1 block text-xs text-content-muted">
              Jika aktif, client bisa memesan komisi langsung dari profil dan artwork kamu.
            </span>
          </span>
        </label>
      </div>
  ...
  useEffect(() => {
    ...
    if (!globalOpen || config?.id !== modalId) {
      openModal({
        ...
        description: "Perbarui bio, harga, dan status ketersediaan komisi kamu.",
        content,
