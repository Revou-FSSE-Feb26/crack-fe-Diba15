...
export default function Stat({
  ...
  const toneIconColor = tone === "primary" ? "text-primary" : "text-content-muted";

  if (variant === "card") {
    return (
      <div className={`rounded-2xl border border-content/10 bg-surface p-4 ${className}`}>
        <div className="flex items-center justify-between gap-3">
          <div>
            {label && <p className="text-xs font-medium text-content-muted">{label}</p>}
            <p className="mt-1 font-display text-2xl font-bold text-content">{displayValue}</p>
          </div>
  ...
  if (variant === "chip") {
    if (iconPlacement === "left") {
      return (
        <div className={`flex items-center gap-2 rounded-xl px-3 py-2 ${toneBg} ${className}`}>
          <Icon className={`w-4 h-4 shrink-0 ${toneIconColor}`} />
          <div className="min-w-0">
            <p className="text-xs text-content-muted">{label}</p>
            <p className="truncate text-sm font-medium text-content capitalize">{displayValue}</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className={`rounded-xl px-3 py-2 ${toneBg} ${className}`}>
        ...
        <p className="mt-1 text-sm font-medium text-content truncate">{displayValue}</p>
      </div>
    );
  }
  ...
  // variant === "inline"
  return (
    <div className={`flex items-center gap-1.5 text-content-muted ${className}`}>
