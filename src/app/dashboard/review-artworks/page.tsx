...
import { CheckCircle2, Clock3, ImageIcon, Search, ShieldAlert, XCircle } from "lucide-react";
...
export default function ReviewArtworksPage() {
  ...
  const { artworks, artworkTags, tags, approveArtwork, rejectArtwork } = useArtworkStore();
  const { openModal } = useModalStore();
  const { addToast } = useToastStore();

  const [search, setSearch] = useState("");
  const [rejectTarget, setRejectTarget] = useState<ArtworkWithRelations | null>(null);
  ...
  const recentReviews = useMemo(
    () =>
      artworksWithRelations
        .filter((item) => item.curation_status === "approved" || item.curation_status === "rejected")
  ...
  if (!isCurator()) {
    return (
      <div className="rounded-2xl border border-content/10 bg-surface p-6 text-center">
        ...
        <h1 className="font-heading text-2xl font-semibold text-content">Akses Kurator Diperlukan</h1>
      ...
      <div className="space-y-4">
        <div className="rounded-2xl border border-content/10 bg-surface p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h1 className="font-heading text-2xl font-semibold text-content">Review Artworks</h1>
              <p className="mt-1 text-sm text-content-muted">
                Periksa artwork yang memilih opsi kurasi. Setujui karya yang memenuhi standar
                atau tolak dengan alasan yang jelas jika tidak lolos review.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-xl border border-premium/20 bg-premium/5 px-4 py-3">
            ...
            <p className="mt-1 font-display text-2xl font-bold text-content">{pendingArtworks.length}</p>
          </div>
          <div className="rounded-xl border border-verified/20 bg-verified/5 px-4 py-3">
            ...
            <p className="mt-1 font-display text-2xl font-bold text-content">
              {artworks.filter((item) => item.curation_status === "approved").length}
            </p>
          </div>
          <div className="rounded-xl border border-danger/20 bg-danger/5 px-4 py-3">
            ...
            <p className="mt-1 font-display text-2xl font-bold text-content">
              {artworks.filter((item) => item.curation_status === "rejected").length}
            </p>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-content/10 bg-surface">
          <div className="flex flex-col gap-3 border-b border-content/10 p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="font-heading text-lg font-semibold text-content">Antrian Pending</h2>
              <p className="text-sm text-content-muted">
                Hanya artwork dengan status pending yang meminta pemeriksaan kurator.
              </p>
            </div>
          ...
          <div className="space-y-4 p-4">
            {pendingArtworks.length === 0 ? (
              <div className="rounded-xl border border-dashed border-content/15 px-4 py-12 text-center">
                ...
                <p className="font-medium text-content">Tidak ada artwork pending</p>
                <p className="mt-1 text-sm text-content-muted">
                  Semua antrian review sudah diproses. Karya baru akan muncul di sini setelah artist mengunggah
                  dengan opsi pemeriksaan kurator.
                </p>
              </div>
            ) : (
        ...
        {recentReviews.length > 0 && (
          <div className="rounded-2xl border border-content/10 bg-surface p-4">
            <h2 className="font-heading text-lg font-semibold text-content">Riwayat Review Terbaru</h2>
            <p className="mt-1 text-sm text-content-muted">
            ...
            <div className="mt-4 divide-y divide-content/10">
              {recentReviews.map((artwork) => (
                <div key={artwork.id} className="flex flex-col gap-2 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-medium text-content">{artwork.title}</p>
                      <span
                        className={[
                      ...
                      >
                        {artwork.curation_status === "approved" ? "Disetujui" : "Ditolak"}
                      </span>
                    </div>
                    ...
                    {artwork.curation_status === "rejected" && artwork.rejection_reason && (
