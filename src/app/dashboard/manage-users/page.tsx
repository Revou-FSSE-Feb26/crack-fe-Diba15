...
export default function ManageUsersPage() {
  ...
  const { users, createCurator, updateUser, deleteUser } = useUserManagementStore();
  ...
  if (!isAdmin()) {
    return (
      <div className="rounded-2xl border border-content/10 bg-surface p-6 text-center">
        ...
        <h1 className="font-heading text-2xl font-semibold text-content">Akses Admin Diperlukan</h1>
      ...
      <div className="w-full space-y-4">
        <div className="flex flex-col gap-4 rounded-2xl border border-content/10 bg-surface p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-heading text-2xl font-semibold text-content">Manage Users</h1>
              <p className="text-sm text-content-muted">
                Kelola seluruh user platform. Penambahan user baru hanya untuk role curator.
              </p>
            </div>
          </div>

          <Button className="flex items-center justify-center gap-1" onClick={openCreateForm}>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          {(Object.keys(roleLabels) as UserRole[]).map((role) => (
            ...
            >
              ...
              <p className="mt-1 font-display text-xl font-bold text-content">{roleCounts[role]}</p>
            </button>
          ))}
          <button
            type="button"
          ...
          >
            ...
            <p className="mt-1 font-display text-xl font-bold text-content">{roleCounts.all}</p>
          </button>
        </div>
